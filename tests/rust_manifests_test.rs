...
const SNAPSHOT_PATH_DYN: &str = "../snapshots/rust-dynamic/";
const SNAPSHOT_PATH_STATIC: &str = "../snapshots/rust-static/";

#[test]
fn test_rust_dynamic() {
    let api_list = vec![
        "write_on_drive",
        "access_network",
        "access_webcam",
    ...
    let folder = tmp_dir.join("rust-dynamic");
    let path = folder.to_str().unwrap();
    create_dir_all(path).unwrap();

    elf_analysis(
        "./tests/elf_file/fake-firmware-rust-dynamic",
        api_list,
        path,
    )
    .unwrap();
...
fn test_rust_static() {
    ...
    let tmp_dir = temp_dir();
    let folder = tmp_dir.join("rust-static");
    let path = folder.to_str().unwrap();
    create_dir_all(path).unwrap();

    elf_analysis("./tests/elf_file/fake-firmware-rust-static", api_list, path).unwrap();
